...
struct Node{
    string nodeSrc;
    vector<string> nodeDest;    //Stores all neighbors
    vector<int> weights;        //Stores the weight of each neighbor
};


/*
 * Struct for each dijkstraNode, used to find shortest path
 */
struct dijkstraNode{
    string label;
     ...
     * exists.
     */
    Node findNode(string const &node_label);
    dijkstraNode findDijkstraNode(string const &node_label);
